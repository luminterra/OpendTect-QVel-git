...
void QVelVolumeBuilder::initWellTracks()
{
    ...
    for (int iwell=0; iwell < numWells_; iwell++)
    {
     ...
     wd_[iwell] = Well::MGR().get( well, false );
...
float  QVelVolumeBuilder::intersectWell(MultiID well, int &irow, int &icol, float &tval, Array2DImpl<float>*  horz)
{
    ...
    // if no intersection, use above layer
...
void QVelVolumeBuilder::computeWellIntersections(TypeSet<MultiID> selectedWells, int numWells, int ihorz, Coord3 *wellIntersections,
    Array2DImpl<float>* topH, Array2DImpl<float>* botH)
{
	...
	bool fail = false;
	    irow = icol = -1;
            ...
            Well::Data* wd = Well::MGR().get( selectedWells[iwell], false );
    ...
    //in 4.6 this wrecks it timetrack.toTime( *(wd->d2TModel()) ,timetrack);
...
#if 0
	    if (tempH->isLoaded())
		if (tracef != NULL) fprintf(tracef,"is loaded\n");
	    if (tempH->isFullyLoaded())
		if (tracef != NULL) fprintf(tracef,"is f loaded\n");
	    else
		if (tracef != NULL) fprintf(tracef,"is NOT f loaded\n");
#endif
	    //Array2D<float>* tempA = tempH->createArray2D(tempH->sectionID(0),0);
	    whpos.setHorizon( emid );
	    ...
	    const Coord3& crd0 = timetrack.getPos( 0. );
            const BinID& bid0 = SI().transform( crd0 );
#if 1
	    // this can happen on a truncation -- use above horizon value
	    if (mUdf(float)==tval) {
    ...
    // and adjust to possibly decimated subset cs_
		...
		icol0/= cs_.hrg.step.crl();
	       ...
	       tval = botH->get(irow0,icol0);
	       //if (tracef != NULL) fprintf(tracef,"Well %d Intersect fail, fix with %g at %d %d\n",iwell,tval,irow0,icol0);
    ...
    // and adjust to possibly decimated subset cs_
      ...
      //if (tracef != NULL) fprintf(tracef,"horizon %d well %d intersec %d %d time %g\n",ihorz, iwell, irow, icol, tval);
      
     // if (tracef != NULL) fprintf(tracef,"Finally: for horizon %d well %d interscetion= %g \n\n",ihorz,iwell,wellIntersections[iwell].z);
...
// with conformant interpolation
 void QVelVolumeBuilder::getWellHorizonVelocities(int ihorz, double dt, int nrows, int ncols, int irow, int icol,
   float topTime, float botTime,Coord3  **wellIntersections, float* velTrace, Array2DImpl<double> *weightsTable)
{
				       ...
				       )
{
	...
	Well::Data* wd = Well::MGR().get( selectedWells_[iwell], false );
	fprintf(tracef," %s\n",wd->name());

	intersectWell(selectedWells_[iwell], irow, icol, tval, NULL);
	if (tracef != NULL) fprintf(tracef,"  intersection result %d %d time=%g\n",irow,icol,tval);
	hitrow = irow;
	hitcol = icol;
#if 0 // don't care about intersection t
	if (mIsUdf(tval)) 
	    continue;
#endif
	if (irow < 0 || icol < 0) 
	...
	botPickZ = markerDepth(botH, horizonName, selectedWells_[iwell], irow, icol, useMarkers,hzmarkersList_);
				      ...
				      StepInterval<int> colrg)
{
	...
	for ( int col=icolbeg, ic=icb; col<=icolend; col+=cs_.hrg.step.crl(), ic++ )
	{
	    ...
	    //if (tracef != NULL) fprintf(tracef,"hz get %d %d to %d %d\n",irow,icol,ir,ic);
	    if (!mIsUdf(t))
		outH->set(ir, ic, t);
	    icend = ic;
	}
	...
	for (int ihorz=0; ihorz < numHoriz; ihorz++) {
		...
		for (int irow = 0; irow < nrows_; irow++)
			for (int icol = 0; icol < ncols_; icol++)
			{
				...
				if (mIsUdf(val)) {
					// find horizon time nearest and above to establish zero-thickness
					float trepl = 0;
					float dx = 1.e10;
					//fprintf(tracef,"missing for horizon %d at %d %d\n",ihorz,irow,icol);
						for (int jhorz=0; jhorz < numHoriz; jhorz++) {
							float hval = horizonTimes_[jhorz]->get(irow, icol);
							if (!mIsUdf(hval)) {
								float testdx = horizonTmin[ihorz]-hval;
								//fprintf(tracef,"   testdx for vs hz %d is %g\n",jhorz,testdx);
								if (testdx >= 0. && testdx < dx) {
									dx = testdx;
									//fprintf(tracef,"   !!!set missing value, tmin to %g\n",hval);
									horizonTimes_[ihorz]->set(irow, icol, hval);
									if (hval < tmin) tmin = hval;
								}
							}
							float ohval = horizonTimes_[ihorz]->get(irow, icol);
							// nothing above that is non-missing
							if (mIsUdf(ohval)) {
								fprintf(tracef,"   horizon %d still has missings\n",ihorz);
								//horizonTimes_[ihorz]->set(irow, icol, tmin);
							}
						}
				}
			}
			fprintf(tracef,"horizon %d tmin %g set to %g\n",ihorz,horizonTmin[ihorz],tmin);
			horizonTmin[ihorz] = tmin;
	}
}
				      ...
				      int    numHoriz)
{
	...
	if (!inserted) { 
		...
		if (tracef) fprintf(tracef,"Hzn tmax %d less than %d\n",i,*it);
    ...
    {
	//int ival = *it;
	fprintf(tracef,"sorted horizon list %d index %d min %g max %g\n",i,*it,horizonTmin[*it],horizonTmax[*it]);
	horizonSortV[i++] = *it;
    }
}
...
int    QVelVolumeBuilder::nextStep()
{
	...
	wellIntersections_ = new Coord3 *[numHorizons_+1];
	for (int i = 0; i < numHorizons_+1; i++) {
	    wellIntersections_[i] = new Coord3 [numWells_];
	...
	{    
	    ...
	    fprintf(tracef,"horizon %d min %g max %g\n",i,horizonTmin_[i],horizonTmax_[i]);
	    delete tempA;
	}
	nrdone_++;
	...
	botH0_ = botH_= new Array2DImpl<float>( nrows_, ncols_ );
	botH_->setAll( 0 );
	...
	topH_ = botH_;
	botH_ =  horizonTimes_[horizonSortV_[ihorz_]];    

	// deal with missing values in horizons
	handleMissing(topH_, botH_, nrows_, ncols_);
	nrdone_++;
	// save well intersections
	computeWellIntersections(selectedWells_, numWells_,ihorz_,wellIntersections_[ihorz_], topH_, botH_);
	// here we go
	computeIntVels(topH_, 
	    botH_, 
	    horizonName, 
	    ihorz_,  
	    botDepths_, 
	...
	// horizon depths can always be recalculated form the model
