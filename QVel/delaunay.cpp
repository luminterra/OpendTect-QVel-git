...
{
  simplex *s = (simplex *)pop(m->deadSimplicies);
...
void removeExternalSimplicies(mesh *m)
{
  ...
  while ((s = (simplex *)nextElement(m->tets, &iter)))
  {
...
simplex** swapSimplexNeighbour(simplex *s, simplex *old, simplex *news)
{
...
void orientationTest(linkedList *tets)
{
  ...
  while((s = (simplex *)nextElement(tets, &iter)))
  {
    vertex *p1, *p2, *p3, *p4;
...
simplex* findContainingSimplex(mesh *m, vertex *p)
{
  ...
  listNode *iter = topOfLinkedList(m->tets);
  simplex  *s    = (simplex *)nextElement(m->tets,&iter); 
  vertex *v1, *v2, *v3, *v4;
...
void addPoint(vertex *p, mesh *m)
{
  ...
  for (j=0; j< arrayListSize(m->conflicts); j++)
  {
     simplex *s = (simplex *)getFromArrayList(m->conflicts,j);
    ...
    for (i=0; i<4; i++)
    {
      ...
      if (! arrayListContains(m->conflicts, s->s[i]))
      {
        ...
        if (o<=0)
        {
          ...
          {
            removeSimplexFromMesh(m, (simplex *)getFromArrayList(m->updates,k));
  ...
  {
    simplex *s = (simplex *)getFromArrayList(m->conflicts, i);
    removeSimplexFromMesh(m,s);
  }
}
...
void setNeighbours(arrayList *newTets)
{
  ...
  {
    s = (simplex *)getFromArrayList(newTets,j);
...
simplex *findAnyNeighbour(vertex *v, arrayList *tets)
{
  ...
  {
    simplex *s = (simplex *)getFromArrayList(tets,i);
...
voronoiCell *newVoronoiCell(mesh *m, int n)
{
  ...
  voronoiCell *vc ;
  vc = (voronoiCell *)pop(m->deadVoronoiCells);

  if (!vc)
  {
    vc = (voronoiCell *)malloc(sizeof(voronoiCell));
  ...
  if (n > vc->nallocated)
  {
    vc->points = (double **)realloc(vc->points, sizeof(double)*n);
    ...
    for (i=vc->nallocated; i<n; i++)
    {
      ...
      VERTEX_MALLOC++;
      #endif
      vc->points[i] = (double *)malloc(sizeof(double)*3);
    }
    vc->nallocated = n;
  }
  vc->n = n;
...
void removePoint(mesh *m)
{
  ...
  for (i=0; i< arrayListSize(m->conflicts); i++)
  {
    s = (simplex *)getFromArrayList(m->conflicts,i);
  ...
  {
    s = (simplex *)getFromArrayList(m->updates, i);
    removeSimplexFromMesh(m,s);
  }
}
...
double voronoiCellVolume(voronoiCell *vc, vertex *p)
{
  ...
  for (i=0; i<arrayListSize(vc->verticies); i++)
  {
    ...
    {     
      thisV = (double *)getFromArrayList(vc->verticies, j);
    ...
    firstV = (double *)getFromArrayList(vc->verticies, i);
    lastV  = NULL;
...
voronoiCell* getVoronoiCell2(vertex *point, simplex *s0, mesh *m)
{
  ...
  for (i=0; i<arrayListSize(neighbours); i++)
  {
    // get this simplex.
    int sIndex = i;
    simplex *s = (simplex *)getFromArrayList(neighbours,i);
    ...
    for (j=0;j<4;j++)
    {
      ...
      if (index != -1)
      {
        ...
        arrayList *thisList = (arrayList *)getFromArrayList(incidentSimplexLists, index);
        int* thisInt = (int *)malloc(sizeof(int));
        *thisInt = sIndex;
  ...
  /* We now have a list of edges which are incident to the point
    ...
    for (i=0; i<arrayListSize(incidentEdges); i++)
    {
      ...
      arrayList *incidentSimplicies = (arrayList *)getFromArrayList(incidentSimplexLists,i);
      ...
      simplex *currentSimplex = (simplex *)getFromArrayList(neighbours, currentIndex);
      ...
      do
      {
        for (k=0; k<arrayListSize(incidentSimplicies); k++)
        {
          ...
          simplex *thisSimplex = (simplex *)getFromArrayList(neighbours, thisIndex);
  ...
  for (i=0; i<arrayListSize(incidentEdges); i++)
  {
    arrayList *thisList = (arrayList *)getFromArrayList(incidentSimplexLists, i);
...
voronoiCell* getVoronoiCell(vertex *point, simplex *s0, mesh *m)
{
  ...
  for (i=0; i<arrayListSize(neighbours); i++)
  {
    s = (simplex *)getFromArrayList(neighbours, i);
    vertex *v1, *v2, *v3;
...
void writeVoronoiCellToFile(FILE* f, voronoiCell *vc)
{
  ...
  for (i=0; i<arrayListSize(vc->verticies); i++)
  {
    vertex *v = (vertex *)getFromArrayList(vc->verticies, i);
...
void undoNeighbourUpdates(neighbourUpdate *nu)
{
  ...
  {
    thisPtr     = (simplex **)pop(nu->ptrs);
    thisSimplex = (simplex *)pop(nu->old);  
...
{
  neighbourUpdate *nu = (neighbourUpdate *)malloc(sizeof(neighbourUpdate));
  nu->ptrs = newStack();
  nu->old  = newStack();
  return nu;
}
...
void freeMesh(mesh *m)
{
  ...
  {
    voronoiCell *vc = (voronoiCell *)pop(m->deadVoronoiCells);
    int i;
